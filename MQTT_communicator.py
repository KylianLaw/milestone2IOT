...
# Ilian Adeleke (2330261) — Lab 8 modules — MQTT_communicator.py
import json
import logging
...
class MQTT_communicator:
    ...
    def send_to_adafruit_io(self, feed_name, value) -> bool:
        """Publish a value to a specific Adafruit IO feed via MQTT."""
    ...
    def stop(self):
        """Cleanly stop MQTT networking loop and disconnect."""
        try:
            if self.mqtt_client:
                self.mqtt_client.loop_stop()
                self.mqtt_client.disconnect()
        except Exception as e:
            logger.debug("MQTT stop error: %s", e)
